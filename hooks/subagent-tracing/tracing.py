...
METADATA_KEY_AGENT_ID = "mlflow.claude_code_agent_id"
METADATA_KEY_AGENT_TYPE = "mlflow.claude_code_agent_type"
METADATA_KEY_PARENT_SESSION = "mlflow.claude_code_parent_session"
...
def _finalize_trace(
    ...
    try:
        ...
        with InMemoryTraceManager.get_instance().get_trace(parent_span.trace_id) as in_memory_trace:
            ...
            if agent_id:
                metadata[METADATA_KEY_AGENT_ID] = agent_id
            if agent_type:
                metadata[METADATA_KEY_AGENT_TYPE] = agent_type
...
def process_transcript(
    ...
    Args:
        ...
        agent_id: Optional subagent identifier (set for subagent transcripts)
        agent_type: Optional subagent type (e.g. "Explore", "Plan")
    ...
    try:
        ...
        span_name = (
            f"claude_code_subagent_{agent_type}" if agent_type else "claude_code_conversation"
        )
        span_inputs = {"prompt": extract_text_content(last_user_prompt)}
        if agent_id:
            span_inputs["agent_id"] = agent_id
        if agent_type:
            span_inputs["agent_type"] = agent_type
