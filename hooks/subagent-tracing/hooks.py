...
def setup_hooks_config(settings_path: Path) -> None:
    ...
    Creates or updates Stop and SubagentStop hooks that call MLflow tracing handlers.
    ...
    upsert_hook(config, "SubagentStop", "subagent-stop-hook")
...
def subagent_stop_hook_handler() -> None:
    """CLI hook handler for subagent completion - processes subagent transcript and creates trace."""
    if not is_tracing_enabled():
        response = get_hook_response()
        print(json.dumps(response))  # noqa: T201
        return
    ...
    try:
        hook_data = read_hook_input()
        session_id = hook_data.get("session_id")
        transcript_path = hook_data.get("agent_transcript_path") or hook_data.get(
            "transcript_path"
        )
        ...
        get_logger().log(
            CLAUDE_TRACING_LEVEL,
            "SubagentStop hook: session=%s, agent_id=%s, agent_type=%s, transcript=%s",
            session_id,
            agent_id,
            agent_type,
            transcript_path,
        ...
        trace = process_transcript(
            transcript_path, session_id, agent_id=agent_id, agent_type=agent_type
        )
        ...
        if trace is not None:
            ...
            response = get_hook_response(
                error=(
                    "Failed to process subagent transcript, please check"
        ...
        get_logger().error("Error in SubagentStop hook: %s", e, exc_info=True)
        response = get_hook_response(error=str(e))
        print(json.dumps(response))  # noqa: T201
        sys.exit(1)
